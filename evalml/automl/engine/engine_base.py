    ...
    for i, (train, valid) in enumerate(automl.data_splitter.split(X_pd, y_pd)):
        ...
        try:
            ...
            if automl.optimize_thresholds and automl.objective.is_defined_for_problem_type(automl.problem_type) and \
                ...
                X_train, X_threshold_tuning, y_train, y_threshold_tuning = split_data(X_train, y_train,
                                                                                      automl.problem_type,
                                                                                      test_size=0.2,
            ...
            if isinstance(e, PipelineScoreError):
                ...
                scores = OrderedDict(
                    {o.name: scores[o.name] for o in [automl.objective] + automl.additional_objectives})
                score = scores[automl.objective.name]
            else:
                score = np.nan
                scores = OrderedDict(
                    zip([n.name for n in automl.additional_objectives], [np.nan] * len(automl.additional_objectives)))
...
class EngineBase(ABC):
    ...
    def train_and_score_pipeline(pipeline, automl, full_X_train, full_y_train):
        ...
        Returns:
            dict: a dict containing cv_score_mean, cv_scores, training_time and a cv_data structure with details.
        """
        return train_and_score_pipeline(pipeline, automl, full_X_train, full_y_train)
